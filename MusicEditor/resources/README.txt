...
Changes made to code:
 - Added sequence functionality to midi so that it can be paused and played at different times. The midi can still use the receiver if the sequence is unavailable
- Added field to GUI to keep track of the Panel so that it can scroll it
- Added a field to the ConcreteGuiViewPanel so that it can keep track of the preview square that is added through GUI input, also added a field to keep track of max beats of the model for use by the red progression line
- Added functions to ConcreteGuiViewPanel and GUI to allow them to implement key and mouse functions such as note previewing, pausing, and restarting
- Added functions to Midi that allow it to be paused and restarted
- Switched GUI to implement the GuiViewInterface interface
- Added method to model to add a note with specific parameters, this is just to make the code simpler
